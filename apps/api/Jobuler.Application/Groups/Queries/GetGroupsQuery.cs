...
public record GroupMemberDto(Guid PersonId, string FullName, string? DisplayName, bool IsOwner, string? PhoneNumber, string? InvitationStatus, string? ProfileImageUrl);
    ...
    public async Task<List<GroupMemberDto>> Handle(GetGroupMembersQuery req, CancellationToken ct)
    {
        ...
        return memberships
            ...
            .Select(m => {
                ...
                return new GroupMemberDto(p.Id, p.FullName, p.DisplayName, m.IsOwner, p.PhoneNumber, p.InvitationStatus ?? "accepted", p.ProfileImageUrl);
            })
            .ToList();
    }
}
